-- Garam Ping Lagger
-- Optimized ping spammer with auto-brainrot detection
-- PC + Controller support | Auto-save config
...
-- CONFIG
-- ══════════════════════════════════════════════════════════════════════
local CONFIG_FILE = "GaramPingLagger_Config.json"
...
local function loadConfig()
    ...
    for k, v in pairs(DEFAULT_CFG) do
        cfg[k] = (type(data[k]) == type(v)) and data[k] or v
    end
end
...
-- ══════════════════════════════════════════════════════════════════════
local C = {
    bg = Color3.fromRGB(8, 4, 20),
    panel = Color3.fromRGB(14, 8, 32),
    card = Color3.fromRGB(22, 12, 46),
    purple1 = Color3.fromRGB(120, 40, 200),
    purple2 = Color3.fromRGB(170, 90, 240),
    purple3 = Color3.fromRGB(195, 130, 255),
    glow = Color3.fromRGB(80, 20, 160),
    white = Color3.fromRGB(235, 225, 255),
    dim = Color3.fromRGB(130, 110, 170),
    green = Color3.fromRGB(100, 255, 160),
    red = Color3.fromRGB(255, 100, 120),
    waiting = Color3.fromRGB(255, 190, 50),
    inputBg = Color3.fromRGB(18, 8, 40),
}

local T = { bg = 0.35, panel = 0.30, card = 0.28, header = 0.12, inputBg = 0.20 }

-- ══════════════════════════════════════════════════════════════════════
-- DESTROY OLD GUI
-- ══════════════════════════════════════════════════════════════════════
for _, kid in pairs(plrGui:GetChildren()) do
    if kid.Name == "GaramPingLaggerGui" then kid:Destroy() end
end

local screen = Instance.new("ScreenGui")
screen.Name = "GaramPingLaggerGui"
...
applyGradient(mainFrame, C.bg, Color3.fromRGB(18, 6, 42), 160)
...
titleLbl.Text = "GARAM PING LAGGER"
...
statusPill.BackgroundColor3 = Color3.fromRGB(40, 10, 60)
...
settingsBtn.BackgroundColor3 = Color3.fromRGB(40, 10, 60)
...
settingsBtn.MouseEnter:Connect(function()
    tw(settingsBtn, { BackgroundColor3 = C.purple1 }, 0.1)
end)
settingsBtn.MouseLeave:Connect(function()
    tw(settingsBtn, { BackgroundColor3 = Color3.fromRGB(40, 10, 60) }, 0.1)
end)
...
applyGradient(settingsFrame, C.panel, Color3.fromRGB(10, 4, 28), 160)
...
setCloseBtn.BackgroundColor3 = Color3.fromRGB(40, 10, 60)
...
setCloseBtn.MouseEnter:Connect(function()
    tw(setCloseBtn, { BackgroundColor3 = C.red }, 0.1)
end)
setCloseBtn.MouseLeave:Connect(function()
    tw(setCloseBtn, { BackgroundColor3 = Color3.fromRGB(40, 10, 60) }, 0.1)
end)
...
local function mkInputRow(yPos, labelText, getValue, onConfirm)
    ...
    box.Focused:Connect(function()
        tw(bs, { Color = C.purple2, Transparency = 0 }, 0.12)
    end)
...
local function mkKeybindRow(yPos, labelText, which)
    ...
    bindBtn.MouseEnter:Connect(function()
        tw(bStr, { Color = C.purple2, Transparency = 0 }, 0.1)
    end)
    bindBtn.MouseLeave:Connect(function()
        tw(bStr, { Color = C.glow, Transparency = 0.5 }, 0.1)
    end)
    ...
    local cStr = Instance.new("UIStroke", clearBtn)
    cStr.Color = C.red
    cStr.Thickness = 1
    cStr.Transparency = 0.5
    clearBtn.MouseEnter:Connect(function()
        tw(cStr, { Transparency = 0 }, 0.1)
    end)
    clearBtn.MouseLeave:Connect(function()
        tw(cStr, { Transparency = 0.5 }, 0.1)
    end)
...
resetBtn.MouseEnter:Connect(function()
    tw(resetBtn, { BackgroundColor3 = C.purple1 }, 0.1)
end)
resetBtn.MouseLeave:Connect(function()
    tw(resetBtn, { BackgroundColor3 = C.glow }, 0.1)
end)
...
applyGradient(confirmBox, C.panel, Color3.fromRGB(10, 4, 28), 160)

local cStroke = Instance.new("UIStroke", confirmBox)
cStroke.Color = C.purple1
...
confirmYes.MouseEnter:Connect(function()
    tw(confirmYes, { BackgroundColor3 = C.purple1 }, 0.1)
end)
confirmYes.MouseLeave:Connect(function()
    tw(confirmYes, { BackgroundColor3 = C.glow }, 0.1)
end)
confirmNo.MouseEnter:Connect(function()
    tw(confirmNo, { TextColor3 = C.white }, 0.1)
end)
confirmNo.MouseLeave:Connect(function()
    tw(confirmNo, { TextColor3 = C.dim }, 0.1)
end)
...
local function openSettings()
    ...
    tw(settingsBtn, { BackgroundColor3 = C.purple1 }, 0.12)
    powerBox.Text = tostring(cfg.power)
    intervalBox.Text = tostring(cfg.interval)
    updateKbLabels()
    if autoBrainrotBtn then
...
local function closeSettings()
    ...
    tw(settingsBtn, { BackgroundColor3 = Color3.fromRGB(40, 10, 60) }, 0.12)
    hideConfirm()
end
...
local function flipLag(state, isManual)
    ...
    if active then
        ...
        statusLbl.Text = "OFF"
        statusLbl.TextColor3 = C.red
        tw(statusPill, { BackgroundColor3 = Color3.fromRGB(40, 10, 60) }, 0.2)
        mainStroke.Color = C.purple1
        mainStroke.Transparency = 0.35
    end
end
...
activateBtn.MouseEnter:Connect(function()
    if not active then tw(activateBtn, { BackgroundColor3 = Color3.fromRGB(90, 30, 150) }, 0.1) end
end)
...
updateKbLabels()# Gara-ping-lagger
Ping lagger 
